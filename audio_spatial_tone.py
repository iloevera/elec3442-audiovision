...
class SpatialTone:
    ...
    def _render_stereo_block(self, frames: int) -> np.ndarray:
        ...
        sample_idx = np.arange(frames, dtype=np.float64)
        phases = phase_start + sample_idx * phase_inc
        mono = np.sin(phases)

        left_gain, right_gain = self._azimuth_to_equal_power_gains(azimuth_deg)
        stereo = np.zeros((frames, 2), dtype=np.float32)
        stereo[:, 0] = (mono * left_gain * volume).astype(np.float32)
        stereo[:, 1] = (mono * right_gain * volume).astype(np.float32)
        return stereo
